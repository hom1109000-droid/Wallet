...
function App() {
  ...
  return <main className="app-shell">
    ...
    {recoveryStep === 2 && <>
      ...
      <section className="workspace"><div className="section-heading"><span>02B</span><div><h2>Amounts to review</h2></div></div><div className="form-card"><div className="network-list">{tokens.length > 0 ? tokens.map(asset => <div key={`${asset.chainId}:${asset.address}`} style={{display:'flex',justifyContent:'space-between',gap:'16px',padding:'12px 0',borderBottom:'1px solid rgba(127,127,127,.12)'}}><span><strong>{asset.name}</strong><small style={{display:'block',opacity:.65}}>{asset.chainName} · {asset.symbol}</small></span><strong>{asset.balance} {asset.symbol}</strong></div>) : <div className="status-line"><span className="status-dot"/>{scanning ? 'Scanning balances…' : 'No discovered balances to review.'}</div>}</div><button className="wide-button" onClick={makePreview} disabled={busy || !address || tokens.length === 0}>Review transaction <span>→</span></button><div className="status-line"><span className="status-dot"/>{status}</div></div></section>
