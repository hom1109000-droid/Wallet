...
function App() {
  ...
  async function finishConnection(eip1193: any, existingAccount?: string) {
    ...
    setAddress(accounts[0] ?? ''); setConnectedChain(chainId); setSelectedChain(chainId); setTokens([]); setScanErrors([]); setScanSummary({scanned:0,successful:0,assets:0}); setRecoveryStep(accounts[0] ? 1 : 1); setStatus(accounts[0] ? 'Wallet connected. Scanning all supported networks…' : 'No wallet account was returned.');
