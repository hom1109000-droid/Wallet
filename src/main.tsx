...
function App() {
  ...
  return <main className="app-shell">
    ...
    {recoveryStep === 2 && <>
      ...
      <section className="workspace"><div className="section-heading"><span>02B</span><div><h2>Amount to review</h2><p>Enter the native-asset amount you want to review before signing.</p></div></div><div className="form-card"><label>Amount <span>({chainInfo?.native ?? 'native asset'})</span></label><input value={amount} onChange={e=>{setAmount(e.target.value);setPreview(false);setTxHash('')}} placeholder="0.00" inputMode="decimal"/><button className="wide-button" onClick={makePreview} disabled={busy || !address}>Continue to confirmation <span>→</span></button><div className="status-line"><span className="status-dot"/>{status}</div></div></section>
    ...
    {recoveryStep === 3 && <>
      <section className="hero-section"><div className="hero-copy"><div className="status-pill"><span className="live-dot"/> STEP 3 OF 3 · REVIEW</div><h1>Review before<br/><em>you sign.</em></h1><p>Everything is shown before the wallet is asked to authorize the transaction. Check the destination, amount, and network in both places.</p></div><div className="hero-card"><div className="hero-card-top"><span>WALLET CONFIRMATION</span><span>●</span></div><div className="security-icon">✓</div><strong>Nothing signed yet</strong><p>Your wallet remains the final approval screen for the exact transaction.</p></div></section>
      <section className="review-card"><div className="section-heading"><span>03</span><div><h2>Final confirmation</h2><p>Review the exact transaction before confirming it in your wallet.</p></div></div><div className="review-grid"><div><small>FROM</small><p>{address}</p></div><div><small>TO</small><p>{destination}</p></div><div><small>AMOUNT</small><p className="amount-value">{amount} {chainInfo?.native ?? 'native asset'}</p></div><div><small>NETWORK</small><p>{chainInfo?.name ?? 'Detected by wallet'}</p></div></div><div className="review-notice"><span>✓</span><div><b>Verify on your wallet</b><p>Compare the destination, amount and network on the wallet's own confirmation screen before approving.</p></div></div><button className="confirm-button" onClick={sendNativeTransaction} disabled={busy}>{busy ? 'Waiting for wallet…' : 'Confirm in wallet'} <span>→</span></button>{txHash&&<p className="tx-result"><b>Transaction submitted:</b> {chainInfo?<a href={chainInfo.explorer+txHash} target="_blank" rel="noreferrer">View on explorer ↗</a>:txHash}</p>}</section>
    </>}
