...
function App() {
  ...
  return <main className="app-shell">
    ...
    {recoveryStep === 2 && <>
      ...
      <section className="workspace"><div className="section-heading"><span>02B</span><div><h2>Amount to review</h2></div></div><div className="form-card"><label>Detected balances</label>{tokens.length > 0 ? <div className="network-list">{tokens.map(asset => <div key={`${asset.chainId}:${asset.address}`}><strong>{asset.balance} {asset.symbol}</strong><small>{asset.name} · {asset.chainName}</small></div>)}</div> : <div className="status-line"><span className="status-dot"/>{scanning ? 'Scanning balances…' : 'No non-zero balances detected on the selected network.'}</div>}<div className="status-line"><span className="status-dot"/>{status}</div></div></section>
